...
- **`service/server.go`** — MCP server exposing `run` and `change_directory` tools. Uses `sync.Mutex` for thread safety. Holds session state (`workingDir`) that must be set via `change_directory` before running commands.
