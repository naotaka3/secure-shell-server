...
サーバーは 3 つの MCP ツールを公開します：

### `cd`

後続のコマンドの作業ディレクトリを設定します。**`run` ツールを使用する前に必ずこのツールを呼び出す必要があります。** ディレクトリは設定ファイルの `allowedDirectories` 内である必要があります。

| パラメータ | 必須 | 説明 |
|-----------|------|------|
| `path` | はい | 作業ディレクトリとして設定するディレクトリ |

### `run`

現在の作業ディレクトリ（`cd` で設定）でシェルコマンドを実行します。許可されたコマンドのみ実行可能です。**最初に `cd` を呼び出して作業ディレクトリを設定する必要があります。**

| パラメータ | 必須 | 説明 |
|-----------|------|------|
| `commands` | はい | 実行するコマンドのリスト |
| `mode` | いいえ | `"parallel"`（デフォルト）または `"serial"` |

### `pwd`

`cd` で設定された現在の作業ディレクトリを表示します。

### 使用フロー

```
1. cd(path: "/home/user/project")                      -> 作業ディレクトリを設定（最初に必須）
2. run(commands: ["ls -la"])                            -> コマンド実行
3. run(commands: ["echo hello"])                        -> ディレクトリは維持される
4. pwd()                                               -> 現在のディレクトリを確認
5. cd(path: "/tmp")                                    -> ディレクトリ変更
